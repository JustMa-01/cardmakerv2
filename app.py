...
def process_image_endpoint():
    ...
    try:
        ...
        # Validate multipliers are within reasonable bounds
        wishes_font_size_multiplier = max(0.1, min(5.0, wishes_font_size_multiplier))
        name_font_size_multiplier = max(0.1, min(5.0, name_font_size_multiplier))
    except (ValueError, TypeError):
        return jsonify({"error": "Invalid font size multiplier values"}), 400
